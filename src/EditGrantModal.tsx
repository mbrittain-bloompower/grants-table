...
const Field: React.FC<FieldProps> = ({ label, value, onChange, required, placeholder, hint, error }) => (
...
const EditGrantModal: React.FC<EditGrantModalProps> = ({ grant, onSave, onClose, isSubmitting, submitError }) => {
  const [form, setForm] = useState({
    name: grant.name ?? '',
    url: grant.url ?? '',
    geographicEligibility: grant.geographicEligibility ?? '',
    typicalAward: grant.typicalAward ?? '',
    applicationStatus: grant.applicationStatus ?? 'Researching',
    contactEmail: grant.contactEmail ?? '',
    nextDeadline: grant.nextDeadline ?? '',
    eligibilityRequirements: grant.eligibilityRequirements ?? '',
    bloomPowerFitNotes: grant.bloomPowerFitNotes ?? '',
  });
        ...
        <form className="modal-form" onSubmit={handleSubmit} noValidate>
          ...
          <div className="modal-field">
            ...
            <textarea
              ...
              placeholder="e.g. 501(c)(3); must serve youth ages 12–18."
              rows={3}
            />
